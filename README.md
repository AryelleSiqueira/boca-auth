...
- If `BOCA_SYSTEM_USER` and/or `BOCA_ADMIN_USER` are undefined, set with the default values (_system_ and _admin_, respectively) or have their values also included in the `BOCA_LOCAL_USERS`, these users will be created automatically and will authenticate using the default method (password).
This means that these variables must be used carefully as they can bypass the value in the `BOCA_AUTH_METHOD` env variable;
